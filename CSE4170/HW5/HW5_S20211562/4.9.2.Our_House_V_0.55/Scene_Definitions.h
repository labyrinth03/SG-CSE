...
struct Shader {
	...
	GLint loc_eye_light_pos_es;     // 눈 공간 광원 위치 
	...
	Shader() {
		...
		loc_ModelMatrix = -1; // 모델 행렬 
		loc_ViewMatrix = -1;
		loc_ModelViewMatrix = -1; 
		...
		loc_light_ambient = -1;  
		loc_light_diffuse = -1;  
		loc_light_specular = -1;
		loc_light_on = -1;


		loc_eye_light_pos_es = -1;     // 눈 공간 광원 위치 (ES: Eye Space)
		loc_eye_light_ambient = -1;    
		loc_eye_light_diffuse = -1;   
		loc_eye_light_specular = -1;  
		loc_eye_light_direction = -1; 
		loc_eye_light_cutoff = -1;       
		loc_eye_light_outer_cutoff = -1; 
...
struct Dynamic_Object { // an object that moves
	...
	void toggle_current_frame_light(); 
};
...
struct Scene {
	...
	SHADER_ID current_shading_mode; 
	...
	Scene() {
		...
		world_light_pos = glm::vec3(120.0f, 80.0f, 50.0f); 
