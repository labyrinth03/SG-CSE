	...
	for (int i = 0; i < cur_object.instances.size(); i++) {
		...
		if (current_shader) {
			...
			if (shader_kind == SHADER_GOURAUD || shader_kind == SHADER_PHONG) {
				...
				// [핵심 수정] 이제 복잡한 계산 대신 Scene에 저장된 값을 직접 사용합니다.
				// 1. Scene에 미리 계산된 광원의 '월드' 정보를 가져옵니다.
				// 2. 이 월드 정보를 '현재 뷰포트의 ViewMatrix'로 변환합니다.
				//glm::vec3 transformed_eye_light_pos_es = glm::vec3(ViewMatrix * glm::vec4(scene.eye_light_world_pos, 1.0f));
				//glm::vec3 transformed_eye_light_dir_es = glm::normalize(glm::vec3(ViewMatrix * glm::vec4(scene.eye_light_world_dir, 0.0f)));
...
void Dynamic_Object::toggle_current_frame_light() {
	...
	// 첫 번째 프레임의 첫 번째 인스턴스를 기준으로 전체 상태 결정
