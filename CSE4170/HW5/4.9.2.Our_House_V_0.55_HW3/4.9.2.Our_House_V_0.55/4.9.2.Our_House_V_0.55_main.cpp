...
void display(void) {
	...
	// [핵심 수정] 렌더링 루프 시작 전, Scene 객체에 메인 카메라의 최신 월드 정보를 계산하여 저장합니다.
	// 이로써 이 프레임 동안 eye_light_world_pos와 dir은 항상 최신 상태를 유지합니다.
	...
	auto update_wolf_light_state = [&](const glm::mat4& current_view_matrix) {
		if (dynamic_object_ID_mapper[DYNAMIC_OBJECT_WOLF] < scene.dynamic_objects.size()) {
			...
			// Dynamic_Object::draw_object()의 늑대 애니메이션 로직을 여기에 정확히 복사해야 합니다.
			// 아래는 애니메이션이 복잡하지 않다고 가정한 단순화된 버전입니다.

			// ... (Dynamic_Objects.cpp의 DYNAMIC_OBJECT_WOLF case 문 로직을 여기에 삽입) ...
...
void keyboard(unsigned char key, int x, int y) {
	...
	case '5':
		if (dynamic_object_ID_mapper[DYNAMIC_OBJECT_WOLF] < scene.dynamic_objects.size()) {
			...
			wolf_object.toggle_current_frame_light(); // 새로 만든 함수 호출
		}
		break;
	case '6': // 투명 모드 On/Off
	{
		// STATIC_OBJECT_GODZILLA에 해당하는 모든 인스턴스의 투명 모드를 토글
	...
	case '`': // (선택 사항) 기본 Simple Shading으로 돌아가기
