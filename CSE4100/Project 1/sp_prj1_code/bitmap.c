...
struct bitmap
*bitmap_expand(struct bitmap *bitmap, size_t size)                          //비트맵 size만큼 확장
{
  struct bitmap *new_bitmap = bitmap_create(bitmap_size(bitmap)+size);      //확장된 크기 만큼 새로운 bitmap create 

  if (new_bitmap == NULL)
    return NULL;
  for (size_t i = 0; i < bitmap_size(bitmap); i++)                          //기존 bitmap size만큼
    bitmap_set(new_bitmap, i, bitmap_test(bitmap, i));                      //기존 bitmap 정보 new_bitmap으로
  bitmap_destroy(bitmap);                                                   //기존거 삭제
  return new_bitmap;
}

void 
bitmap_print(struct bitmap *bitmap)                                         //bitmap 프린트
{
