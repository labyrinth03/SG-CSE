...
int recommend(RecNode *root){
	...
	for(br = 0; br < rotate[nextBlock[root->level]]; br++){				//br: blockrotate
		for(rx = -4; rx < WIDTH; rx++){										//rx: recommend 상에서의 x좌표
			if(CheckToMove(root->f, currentID, br, 0, rx) == 1){ 			// (rx, 0)좌표에 대해 이동이 가능하다면
				...
				croot->accumulatedScore = root->accumulatedScore + (10*touched) + (deletion*deletion*100);
				recommend(croot);
				num++;
