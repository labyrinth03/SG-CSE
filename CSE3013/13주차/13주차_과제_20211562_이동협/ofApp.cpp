...
void ofApp::freeMemory() {
	...
	if (isBFS) {
		free(atrack); atracknum = 0;
		free(qtrack); qtracknum = 0;
		isBFS = false;
	}
...
bool ofApp::BFS()
{
	...
	init.xpos = j;
	init.ypos = i;
	q.push(init);										//최단거리 stack에 처음위치 push
	allq.push(init);										//모든자취 stack에 처음위치 push
	mark[i][j] = 1;										//처음위치에서 시작하므로 처음위치 방문한 적 있는 것으로 설정

	while (!q.empty()) {

		graph top = q.front();							//최단거리 stack의 top 값
		i = top.ypos; j = top.xpos;
		...
		for (int dir = 0; dir < 4; dir++) {
			if (dir == 0) {
				if (top.right == 1 && mark[i][j + 1] == 0) {		//오른쪽이 열려있고 오른쪽에 간 적이 없다면
					...
					q.push(right);								//최단거리 stack에 push
					allq.push(right);							//모든자취 stack에 push
					mark[i][j + 1] = 1;							//이동한 적 있는 상태로설정
					//j++;										//현재 좌표 이동(오른쪽으로)
				}
			}
	...
	printf("queue completed and printing\n");

	atrack = (graph*)malloc(sizeof(graph) * 10000);								//stack에 있던 graph들을 array에 저장(draw()함수가 프로그램이 동작되는 동안 무한히 작동하기 때문에 drawdfs에서 pop할 수 없음.
	atrack[0].xpos = HEIGHT - 1; atrack[0].ypos = WIDTH - 1; atracknum++;
	while (!allq.empty()) {
		atrack[atracknum] = allq.front();
		printf("track %d %d\n", atrack[atracknum].ypos, atrack[atracknum].xpos);
		allq.pop();
		atracknum++;
	}
	qtrack = (graph*)malloc(sizeof(graph) * 10000);
	qtrack[0].xpos = HEIGHT - 1; qtrack[0].ypos = WIDTH - 1; qtracknum++;
	while (!min.empty()) {
		qtrack[qtracknum] = min.top();
		printf("queue track %d %d\n", qtrack[qtracknum].ypos, qtrack[qtracknum].xpos);
		min.pop();
		qtracknum++;
	}
	return 0;
...
void ofApp::bfsdraw()
{
	...
	for (int j = 0; j < atracknum - 1; j++) {																							//모든자취 그리기
		/*if (abs(atrack[j].xpos - atrack[j + 1].xpos) + abs(atrack[j].ypos - atrack[j + 1].ypos) <= 1) {										//예외방지(그리는 선의 길이가 1 초과인 경우)

			ofDrawLine(30 * atrack[j].xpos + 20, 30 * atrack[j].ypos + 20, 30 * atrack[j + 1].xpos + 20, 30 * atrack[j + 1].ypos + 20);
		}
		else {
			//printf("problem %d %d, tag is %s", track[j].xpos, track[j].ypos, track[j].tag);
			if (!strcmp(atrack[j].tag, "right")) {
				//printf(" right\n");
				ofDrawLine(30 * (atrack[j].xpos - 1) + 20, 30 * atrack[j].ypos + 20, 30 * atrack[j].xpos + 20, 30 * atrack[j].ypos + 20);
			}
			else if (!strcmp(atrack[j].tag, "down")) {
				//printf(" down\n");
				ofDrawLine(30 * atrack[j].xpos + 20, 30 * (atrack[j].ypos - 1) + 20, 30 * atrack[j].xpos + 20, 30 * atrack[j].ypos + 20);
			}
			else if (!strcmp(atrack[j].tag, "up")) {
				//printf(" up\n");
				ofDrawLine(30 * atrack[j].xpos + 20, 30 * (atrack[j].ypos + 1) + 20, 30 * atrack[j].xpos + 20, 30 * atrack[j].ypos + 20);
			}
			else if (!strcmp(atrack[j].tag, "left")) {
				//printf(" left\n");
				ofDrawLine(30 * (atrack[j].xpos + 1) + 20, 30 * atrack[j].ypos + 20, 30 * atrack[j].xpos + 20, 30 * atrack[j].ypos + 20);
			}
			*/
	...
	ofSetColor(0, 225, 0);																													//최단거리 그리기
	if (qtracknum) ofDrawLine(30 * qtrack[0].xpos + 20, 30 * qtrack[0].ypos + 20, 30 * (HEIGHT - 1) + 20, 30 * (WIDTH - 1) + 20);
	for (int j = 0; j < qtracknum - 1; j++) {
		if (abs(qtrack[j].xpos - qtrack[j + 1].xpos) + abs(qtrack[j].ypos - qtrack[j + 1].ypos) <= 1) {

			ofDrawLine(30 * qtrack[j].xpos + 20, 30 * qtrack[j].ypos + 20, 30 * qtrack[j + 1].xpos + 20, 30 * qtrack[j + 1].ypos + 20);
		}
	}
